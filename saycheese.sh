...
banner() {
    ...
    printf " \e[1;77m[\e[1;93m::\e[0m\e[1;77m]      v2.0 mantained by github.com/fauzipadlaw                                  \e[1;77m[\e[1;93m::\e[0m\e[1;77m]\e[0m\n"
...
dependencies() {
    command -v php >/dev/null 2>&1 || {
        echo >&2 "I require php but it's not installed. Install it.(apt install php or https://www.php.net/manual/en/install.php) Aborting."
        exit 1
    }
}
...
ngrok_server() {
    if [[ ! -e ngrok ]]; then
        command -v unzip >/dev/null 2>&1 || {
            echo >&2 "I require unzip but it's not installed. Install it(apt install unzip or http://infozip.sourceforge.net/UnZip.html#Downloads). Aborting."
            exit 1
        }
        command -v wget >/dev/null 2>&1 || {
            echo >&2 "I require wget but it's not installed. Install it(apt install wget or https://www.gnu.org/software/wget/). Aborting."
            exit 1
        }
...
start() {
    ...
    if [[ $option_server -eq 1 ]]; then
        ...
        command -v httrack >/dev/null 2>&1 || {
            echo >&2 "I require httrack: (apt-get install httrack or https://www.httrack.com/page/2/) "
            exit 1
        }
